  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm">
      <h2 className="text-xl font-bold text-stone-800 mb-6 flex items-center">
        ...
        Lista de Invitados ({weddingGuests.length})
      </h2>

      {weddingGuests.length > 0 ? (
        <div className="space-y-4 max-h-96 overflow-y-auto">
          {weddingGuests.map((guest) => {
            const attendanceOption = attendanceOptions.find(
              (opt) => opt.value === guest.attendance
            );
            return (
              <div
                key={guest.id}
                  ...
                  <div className="flex-1">
                    <h3 className="font-semibold text-stone-800 flex items-center">
                      ...
                      <span className="ml-2 text-lg">
                        {attendanceOption?.icon}
                      </span>
                    </h3>
                    <div className="mt-2 space-y-1 text-sm text-stone-600">
                      <p>
                        👥 {guest.guestCount} persona
                        {guest.guestCount > 1 ? 's' : ''}
                      </p>
                      <p>
                        📅{' '}
                        {new Date(guest.eventDate).toLocaleDateString('es-ES')}
                      </p>
                      <p>🕐 {guest.eventTime}</p>
                    </div>
                  </div>
